package com.millennialmedia.android;

import android.graphics.drawable.BitmapDrawable;
import android.view.View;
import android.widget.Button;
import android.widget.MediaController;
import android.widget.FrameLayout.LayoutParams;
import com.millennialmedia.android.Base64;
import com.millennialmedia.android.InlineVideoView;
import com.millennialmedia.android.InlineVideoView$MediaController$MediaClickListener;
import com.millennialmedia.android.MMLog;
import java.io.ByteArrayInputStream;
import java.lang.ref.WeakReference;

class InlineVideoView$MediaController extends MediaController {
   BitmapDrawable inDrawable;
   private WeakReference inlineVideoRef;
   BitmapDrawable outDrawable;

   public InlineVideoView$MediaController(InlineVideoView var1) {
      super(var1.getContext());
      this.inlineVideoRef = new WeakReference(var1);
   }

   public void setAnchorView(View var1) {
      super.setAnchorView(var1);
      Button var5 = new Button(this.getContext());
      if(this.inDrawable == null) {
         try {
            this.inDrawable = new BitmapDrawable(new ByteArrayInputStream(Base64.decode("")));
         } catch (Exception var4) {
            MMLog.e("InlineVideoView", "Exception setting image anchorView inDrawable:", var4);
         }
      }

      if(this.outDrawable == null) {
         try {
            this.outDrawable = new BitmapDrawable(new ByteArrayInputStream(Base64.decode("")));
         } catch (Exception var3) {
            MMLog.e("InlineVideoView", "Exception setting image anchorView outDrawable:", var3);
         }
      }

      InlineVideoView var2 = (InlineVideoView)this.inlineVideoRef.get();
      if(var2 != null) {
         if(var2.inlineParams.goingFullScreen) {
            var5.setBackgroundDrawable(this.inDrawable);
         } else {
            var5.setBackgroundDrawable(this.outDrawable);
         }

         var5.setOnClickListener(new InlineVideoView$MediaController$MediaClickListener(var2));
      }

      LayoutParams var6 = new LayoutParams(-2, -2, 5);
      var6.setMargins(0, 20, 10, 0);
      this.addView(var5, var6);
   }
}
