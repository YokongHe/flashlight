...
final class BakedBezierInterpolator implements Interpolator {
   ...
   public final float getInterpolation(float var1) {
      if(var1 >= 1.0F) {
         ...
         int var3 = Math.min((int)((float)(VALUES.length - 1) * var1), VALUES.length - 2);
         var1 = (var1 - (float)var3 * STEP_SIZE) / STEP_SIZE;
         float var2 = VALUES[var3];
         return (VALUES[var3 + 1] - VALUES[var3]) * var1 + var2;
      }
   }
}
